...
- Workflow steps -- exact numbered steps with CLI commands, model names, delegation targets, and reference files to load
...
Read CLAUDE.md to get the current Custom Skills table, then append the new row:

```
| {trigger phrases}, /{name} | load /{name} skill directly |
```

To append safely without clobbering existing rows, use the full current table block as
old_string and replace it with the same block plus the new row appended at the bottom.

## Step 5: Upload to Skills Marketplace

After writing the local files, upload the skill using whichever skill-publish tool is
